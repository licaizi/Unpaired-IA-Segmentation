...
class UNet(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=6,upsample=False, init_features=32):
        super(UNet, self).__init__()

        features = init_features
        self.encoder1 = UNet._block(in_channels, features, name="enc1")#256x256
        ...
        if upsample:
            self.upconv4 = UNet._upblock(features * 16, features * 8,name='upconv4')
        else:
        ...
        if upsample:
            self.upconv3 = UNet._upblock(features * 8, features * 4,name='upconv3')
        else:
        ...
        if upsample:
            self.upconv2 = UNet._upblock(features * 4, features * 2,name='upconv2')
        else:
        ...
        if upsample:
            self.upconv1 = UNet._upblock(features * 2, features,name='upconv1')
        else:
    ...
    def forward(self, x):
        ...
        return nn.Softmax2d()(self.conv(dec1))
...
model = UNet_add(init_features=32,in_channels=1,out_channels=4,upsample=True)
output = model(data)
print(output.shape)