...
class ResidualBlock(nn.Module):
    def __init__(self, input_channel, output_channel):
        ...
        conv_block = [nn.Conv2d(in_channels=input_channel, out_channels=output_channel,
                                kernel_size=3,
                                padding=1,
                                bias=False),
...
class ModalitySpecificBatchnorm(nn.Module):
    def __init__(self, num_features, num_classes, eps=1e-5, momentum=0.1, affine=True,
                 track_running_stats=True):
        super().__init__()

        self.bns = nn.ModuleList([nn.BatchNorm2d(num_features=num_features, eps=eps, momentum=momentum, affine=affine,
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, input_channel, out_channel, name):
        super().__init__()
...
class SpeciModalConvBlock(nn.Module):

    def __init__(self, input_channel, out_channel, num_domains):
        super().__init__()
        self.conv1 = nn.Conv2d(input_channel, out_channel, kernel_size=3, padding=1)
        self.bn1 = ModalitySpecificBatchnorm(out_channel,num_domains)
        self.leak_relu1 = nn.LeakyReLU(inplace=True)
        self.conv2 = nn.Conv2d(out_channel, out_channel, kernel_size=3, padding=1)
        self.bn2 = ModalitySpecificBatchnorm(out_channel, num_domains)
...
class ModalitySpeciUpBlock(nn.Module):
    def __init__(self, input_channel, out_channel,num_domains):
        super().__init__()
        self.up = Upsample(scale_factor=2, mode='bilinear')
        self.conv = nn.Conv2d(in_channels=input_channel, out_channels=out_channel,
                              kernel_size=3,
                              padding=1,
                              bias=False)
        self.bn = ModalitySpecificBatchnorm(out_channel,num_domains)
