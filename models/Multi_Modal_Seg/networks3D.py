...
from Semi_DA_seg.model_utils import ConvBlock,ResidualConvBlock,DownsamplingConvBlock,UpsamplingDeconvBlock,ResnetBlock,\
     UnetSkipConnectionBlock
from Multi_Modal_Seg.UNet import Unet_3D,SeperateBnUnet_3D
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm3d, use_sigmoid=False):
        ...
        self.first_param = sequence[0]
        self.last_param = sequence[-1]
        if use_sigmoid:
            sequence += [nn.Sigmoid()]
        # sequence += [nn.Tanh()]
...
# model = NLayerDiscriminator(1,64,3).cuda()
...
def define_seg(opt,model_name = 'Unet_3D'):
    args = opt
    ...
    init_net(model, opt.init_type, opt.init_gain,opt.gpu_ids)
...
class UnetGenerator(nn.Module):
    ...
    def forward(self, input):
        return self.model(input)
# model = UnetGenerator(1,1,5,32).cuda()
# data = torch.rand(2,1,128,128,128).cuda()
# out = model(data)
# print(out.shape)
