...
def generate_batch_regions(img_batch, gt_batch, size=16, num_keys=8):
    ...
    # anchor_regions = torch.stack(anchor_regions,dim=0)#b,1,c,h,w,z
from torchvision.models import resnet18

# probs = torch.rand(2,200,4,4).cuda()
# probs_squeeze = probs.squeeze()
...
# model = model_()
# res_ = model(probs)
# print(probs_squeeze.shape,res_.shape,res_.squeeze().shape)
# soft_probs = torch.softmax(probs,dim=1)
# print(soft_probs.shape)
# probs2 = torch.rand(2,200).cuda()
# dist = torch.dist(probs,probs2,p=2)
# print(dist)
# con_index = torch.where(probs[0]>0.5)
# print(con_index)
# con_index = torch.where(probs[1]>0.5)
# print(con_index)
# print(probs.dtype)
# # print(probs)
# out = torch.multinomial(probs,100)
# print(out.shape,probs.shape)
# data = torch.index_select(probs[0],dim=0,index=out[0])

# li = list(map(lambda x:(x-1,x+1),[1,2,3]))
# li = [torch.tensor(2),torch.tensor(3)]
# print(sum(li).data.item()/len(li))
# anchor = torch.rand(2,1,10)
# keys = torch.rand(2,8,10)
# pos = torch.rand(3,4)
# pos2 = torch.rand(3,4)
# pos_sum = torch.cosine_similarity(pos,pos2,dim=1)
# print(pos_sum.shape)
# simi = torch.cosine_similarity(anchor,keys,dim=2)
# print(simi,simi.shape)
# sim_k,index = torch.topk(simi.unsqueeze(1),k=4,dim=2)
# print(sim_k,index,index.shape)
# print(keys[0,index[0,0,:]],keys[0,index[0,0,:]].shape)
# keys = keys.view(1,16,10)
# keys = keys.repeat(2,1,1)
# print(keys,keys.shape)
# print(li)
# for b in li:
#     print(b[0],b[1])
# data = torch.rand(2,16)
# print(data)
# print(torch.norm(data,p=2,dim=1,keepdim=True))
# data = data/torch.norm(data,p=2,dim=1,keepdim=True)
# print(data)
# W,H,Z = 128,128,128
# anchor_border = [56,56,56,16,16,16]
# count = 0
# sampled_borders = []
# count2 = 0
# while count < 8:
...
#     sampled_border = [cx, cy, cz, 16, 16, 16]
#     if judge_iou(sampled_border, anchor_border):
#         # sampled_borders.append(sampled_border)
#         count += 1
#         print(sampled_border)
